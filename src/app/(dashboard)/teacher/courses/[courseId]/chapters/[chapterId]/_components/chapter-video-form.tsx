...
export function ChapterVideoForm({
	...
	courseId,
	chapterId,
}: ChapterVideoFormProps): ReactNode {
	...
	return !isEditing ? (
		<div className={cn("p-4 space-y-2 rounded-lg bg-muted")}>
			...
			{initialData.videoUrl ? (
				<div
					className={cn(
						"relative overflow-hidden rounded-sm h-56 grid place-items-center bg-foreground/25",
					)}
				>
				...
				<div
					className={cn(
						"rounded-sm h-56 grid place-items-center bg-foreground/25",
					)}
				>
					<Video size={40} />
				</div>
